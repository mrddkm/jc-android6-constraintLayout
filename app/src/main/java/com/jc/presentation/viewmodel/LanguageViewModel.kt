...
import com.jc.data.local.datastore.DEFAULT_LANGUAGE_CODE
...
class LanguageViewModel(
    ...
    val currentLanguageCode: StateFlow<String> =
        getSelectedLanguageFlowUseCase()
            .stateIn(
                ...
                initialValue = DEFAULT_LANGUAGE_CODE
            )

    val supportedLanguages = listOf(
        LanguageOption(Constants.LANGUAGE_CODE_ENGLISH, Constants.LANGUAGE_DISPLAY_NAME_ENGLISH),
        LanguageOption(Constants.LANGUAGE_CODE_INDONESIAN, Constants.LANGUAGE_DISPLAY_NAME_INDONESIAN)
    )
    ...
    fun setLanguage(languageCode: String, activityRecreator: () -> Unit) {
        ...
        viewModelScope.launch {
            ...
            Log.d("LanguageViewModel", "Language saved: $languageCode. Current VM state: ${currentLanguageCode.value}")

            val localeList = LocaleListCompat.forLanguageTags(languageCode)
            Log.d("LanguageViewModel", "Setting application locales to: ${localeList.toLanguageTags()}")
    ...
    fun applyInitialLocale() {
        ...
        Log.d("LanguageViewModel", "applyInitialLocale called. Language to apply from StateFlow: $languageCodeToApply")
        val currentAppLocales = AppCompatDelegate.getApplicationLocales()
        Log.d("LanguageViewModel", "Current system app locales: ${currentAppLocales.toLanguageTags()}")
        ...
        if (currentAppLocales.isEmpty || currentAppLocales.toLanguageTags() != languageCodeToApply) {
            ...
            Log.d("LanguageViewModel", "No change needed, current locales already match: $languageCodeToApply")
        }
    }
}
